...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        .route("/{match_id}/result/history", get(results::list_result_claims))
        .route(
        ...
        .route("/{match_id}/evidence/upload", post(evidence::initiate_upload))
        .route("/{match_id}/evidence/link", post(evidence::add_link_evidence))
        .route("/{match_id}/evidence/discover", get(evidence::discover_evidence))
        .route("/{match_id}/evidence/link-discovered", post(evidence::link_discovered_evidence))
        .route("/{match_id}/evidence/validate", post(evidence::validate_evidence))
        // CS2 Demo validation endpoints
        .route("/{match_id}/evidence/validate-demo", post(evidence::validate_demo))
        .route("/{match_id}/evidence/link-demo", post(evidence::link_demo))
        .route("/{match_id}/evidence/demo-stats/{demo_name}", get(evidence::get_demo_stats))
        // Evidence instance endpoints
        .route("/{match_id}/evidence/{evidence_id}", get(evidence::get_evidence))
        .route("/{match_id}/evidence/{evidence_id}", delete(evidence::delete_evidence))
        .route("/{match_id}/evidence/{evidence_id}/complete", post(evidence::complete_upload))
        .route("/{match_id}/evidence/{evidence_id}/access", get(evidence::get_access_url))
        ...
        .route("/{match_id}/result-review", get(result_reviews::get_result_review))
        .route("/{match_id}/result-review/acknowledge", post(result_reviews::acknowledge_result_review))
}
