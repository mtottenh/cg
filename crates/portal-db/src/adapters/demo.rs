...
impl DemoPlayerRepository for PgDemoPlayerRepository {
    ...
    async fn restrict_attribution_to_lineup(
        ...
        // P-25 attribution gate. A demo player counts for the match only if the
        // resolved player is in the match's authoritative (demo-source) lineup.
        // The `has_lineup` guard makes this self-gating: with no demo lineup for
        // the match (season did not opt in), nothing changes and the global
        // attribution stands (pre-cutover fallback).
        //
        // NOTE: this deliberately touches `demo_player_stats` (owned by
        // PgDemoPlayerStatsRepository) as well as `demo_players`, because both
        // are demo-scoped attribution projections and must be gated together;
        // splitting the pass across repos would let the two diverge.
        let mut tx = self
            .pool
            .begin()
            .await
            .map_err(|e| DomainError::internal(format!("begin restrict tx: {e}")))?;
        ...
        let lineup_cte = r"
            WITH lp AS (
                ...
                WHERE ml.match_id = $2 AND mlp.source = 'demo'
            ),
            guard AS (SELECT EXISTS(SELECT 1 FROM lp) AS present)
        ";

        let players_res = sqlx::query(&format!(
            r"{lineup_cte}
            UPDATE demo_players dp SET player_id = NULL
            WHERE dp.demo_id = $1
              AND dp.player_id IS NOT NULL
              AND (SELECT present FROM guard)
              AND dp.player_id NOT IN (SELECT player_id FROM lp)
            "
        ))
        .bind(demo_id.as_uuid())
        .bind(match_id.as_uuid())
        .execute(&mut *tx)
        .await
        .map_err(|e| DomainError::internal(format!("restrict demo_players: {e}")))?;

        let stats_res = sqlx::query(&format!(
            r"{lineup_cte}
            UPDATE demo_player_stats s SET player_id = NULL
            WHERE s.demo_id = $1
              AND s.player_id IS NOT NULL
              AND (SELECT present FROM guard)
              AND s.player_id NOT IN (SELECT player_id FROM lp)
            "
        ))
        .bind(demo_id.as_uuid())
        .bind(match_id.as_uuid())
        .execute(&mut *tx)
        .await
        .map_err(|e| DomainError::internal(format!("restrict demo_player_stats: {e}")))?;

        tx.commit()
            .await
            .map_err(|e| DomainError::internal(format!("commit restrict tx: {e}")))?;

        Ok(players_res.rows_affected() + stats_res.rows_affected())
    }
